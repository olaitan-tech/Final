...
export default function NewOrder() {
  ...
  const { handleSubmit, handleChange, handleBlur, values, errors, touched } = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      if (!session?.user?.email) return alert("You must be logged in to place an order");

      setProgress(true);
      try {
        await addDoc(collection(db, "restaurantOrders"), {
          user: session.user.email, // store email
  ...
  return (
    <main className="min-h max-w-xl mx-auto my-10 px-6 py-8 bg-white shadow-lg rounded-xl">
      <h1 className="text-2xl font-semibold mb-6 text-center text-gray-700">Aldo's Restaurant Place Your Order</h1>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-5">
        ...
        <FormControl fullWidth>
          ...
          {touched.foodItem && errors.foodItem && <span className="text-red-500 text-xs">{errors.foodItem}</span>}
        </FormControl>
        <TextField
          fullWidth
          size="small"
          ...
          onBlur={handleBlur}
        />
        {touched.quantity && errors.quantity && <span className="text-red-500 text-xs">{errors.quantity}</span>}
        <TextField
          fullWidth
          size="small"
          ...
          onBlur={handleBlur}
        />
        {touched.deliveryDate && errors.deliveryDate && <span className="text-red-500 text-xs">{errors.deliveryDate}</span>}
        <TextField
          fullWidth
          size="small"
          label="Total Amount (₦)"
          ...
          onBlur={handleBlur}
        />
        {touched.amount && errors.amount && <span className="text-red-500 text-xs">{errors.amount}</span>}
        <FormControl fullWidth>
          ...
          {touched.status && errors.status && <span className="text-red-500 text-xs">{errors.status}</span>}
        </FormControl>
        <TextField
          fullWidth
          type="text"
          ...
          onBlur={handleBlur}
        />
        {touched.notes && errors.notes && <span className="text-red-500 text-xs">{errors.notes}</span>}

        <button type="submit" className="h-[40px] w-full flex justify-center items-center gap-2 rounded-md shadow-md bg-orange-400 text-white text-lg cursor-pointer">
          Submit Order
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <Typography>Thank you for ordering at <b>Aldo's Restaurant</b></Typography>
        </DialogContent>
        <DialogActions>
          <Link href="/payment"><Button onClick={handleClose} color="primary" variant="contained" autoFocus>Close</Button></Link>
        </DialogActions>
      </Dialog>
    </main>
  );
